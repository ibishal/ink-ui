  ...
  return (
    <div className="flex-1 flex flex-col p-8 gap-8 animate-in fade-in duration-700">
      <nav className="flex justify-between items-center px-4">
        <button 
          onClick={onBack}
          className="group flex items-center gap-4 border border-white/10 bg-white/5 px-8 py-3 text-[11px] font-black uppercase tracking-[0.2em] hover:bg-white hover:text-black transition-all"
        >
          ← BACK_TO_SECTOR
        </button>
      ...
      <div className="flex-1 flex gap-8 overflow-hidden min-h-[600px]">
        ...
        <div className="w-[420px] flex flex-col gap-8">
           <div className="flex-1 border border-white/10 bg-white/5 p-10 overflow-y-auto custom-scrollbar">
              ...
              <div className="space-y-4">
                 <button onClick={onShowDocs} className="w-full py-4 border border-white/10 bg-white/5 text-[11px] font-black uppercase tracking-widest hover:bg-white hover:text-black transition-all">Documentation Access</button>
              </div>
           </div>
        </div>

        {/* Code Mirror */}
        <div className="flex-1 border border-white/10 bg-black/80 flex flex-col overflow-hidden">
        ...
        <div className="w-[350px] flex flex-col gap-8">
           <button className="h-40 bg-ink-pink border border-ink-pink flex flex-col items-center justify-center group shadow-[0_0_60px_rgba(230,0,122,0.1)] active:scale-95 transition-all">
              <ICONS.Zap />
              <span className="text-xl font-black uppercase tracking-widest mt-4">Execute_Pwn</span>
           </button>
           <div className="flex-1 border border-white/10 bg-[#0A0A0B] p-8 flex flex-col">
